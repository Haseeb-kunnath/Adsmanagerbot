...
print_r($result);
            ...
            $b = $read['balance'];
            $f = $b + $ex[2];
            $rwd = mysqli_query($db, "UPDATE channel SET balance = '".$f."' WHERE c_id = '".$ch_id."'");
            if($rwd){
                $bot->send_message($chatid, "✅ Channel rewarded sucessfully.", null, null, 'HTML');
            }else{
                $bot->send_message($chatid, "❌ Failed to reward channel", null, null, 'HTML');
            }
        }else{
